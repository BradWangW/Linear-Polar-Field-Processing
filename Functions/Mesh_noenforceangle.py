...
class Triangle_mesh():
    ...
    def get_V_F_f_E_twin(self):
        # metric M_G_ff
        self.d_G_ff = lil_matrix((len(self.F), len(self.E) * 4), dtype=float)
        self.Area_inv_ff = lil_matrix((len(self.F), len(self.F)), dtype=float)
        ...
        for i, f in tqdm(enumerate(self.F), 
            ...
            for j, k in np.stack([np.arange(3), np.roll(np.arange(3), -1)], axis=1):
                ...
                self.d1_extended[i, i * 3 + j] = 1
                
                self.E_extended_dict[tuple([f_f[j], f_f[k]])] = i * 3 + j
            ...
            self.d_G_ff[i, i * 3] = (z1 + z2).real/2
            self.d_G_ff[i, i * 3 + 1] = (z2 + z0).real/2
            self.d_G_ff[i, i * 3 + 2] = (z0 + z1).real/2
            
            self.Area_inv_ff[i, i] = 1/np.linalg.norm(np.cross(vec0, vec1))
            
        self.V_extended = np.array(self.V_extended)
        
        self.M_G_ff = self.d_G_ff.T @ self.Area_inv_ff @ self.d_G_ff
    
    def get_F_e_E_comb(self):
        # metric M_G_fe
        self.d_G_fe = lil_matrix((len(self.E), len(self.E) * 4), dtype=float)
        self.Area_inv_fe = lil_matrix((len(self.E), len(self.E)), dtype=float)
        ...
        for i, e in tqdm(enumerate(self.E), 
            ...
            if len(e_twin) == 2:
                ...
                self.d1_extended[len(self.F_f) + i, e_twin] = 1
                ...
                self.E_extended_dict[tuple([edges[0][1], edges[1][0]])] = len(self.E_comb) - 2 + len(self.E_twin)
                self.E_extended_dict[tuple([edges[1][1], edges[0][0]])] = len(self.E_comb) - 1 + len(self.E_twin)
                ...
                e_i_j, e_j_k, e_kf_if, e_k_l, e_l_i, e_ig_kg = self.E_extended_dict[tuple([edges[1][1], other_v_f])],\
                    self.E_extended_dict[tuple([other_v_f, edges[1][0]])],\
                    self.E_extended_dict[tuple([edges[1][0], edges[1][1]])],\
                    self.E_extended_dict[tuple([edges[0][1], other_v_g])],\
                    self.E_extended_dict[tuple([other_v_g, edges[0][0]])],\
                    self.E_extended_dict[tuple([edges[0][0], edges[0][1]])]
                ...
                self.Area_inv_fe[i, i] = 2/area_sum
                
                self.d_G_fe[i, e_i_j] = - (z_ik + z_jk).real/2
                self.d_G_fe[i, e_j_k] = (z_ij + z_ik).real/2
                self.d_G_fe[i, e_kf_if] = (z_jk - z_ij).real/2
                self.d_G_fe[i, e_k_l] = (z_ik - z_li).real/2
                self.d_G_fe[i, e_l_i] = (-z_ik + z_kl).real/2
                self.d_G_fe[i, e_ig_kg] = (z_li - z_kl).real/2
                self.d_G_fe[i, e_if_ig] = (z_ik * np.exp(-1j * np.pi/2)).real
                self.d_G_fe[i, e_kg_kf] = (z_ik * np.exp(1j * np.pi/2)).real
                ...
                self.d1_extended[len(self.F_f) + i, len(self.E_twin) + len(self.E_comb) - 2] = 1
                self.d1_extended[len(self.F_f) + i, len(self.E_twin) + len(self.E_comb) - 1] = 1
                ...
                self.rotations_across_e_extended += [
                    rotation, -rotation
                ]
        ...
        # Computing the metric using integral over the edge-faces and face-faces.
        self.d_G_complete = vstack([
            self.d_G_ff, self.d_G_fe
        ])
        self.Area_inv_extended = bmat([
            [self.Area_inv_ff, None],
            [None, self.Area_inv_fe]
        ])
        
        self.M_G_fe = self.d_G_fe.T @ self.Area_inv_fe @ self.d_G_fe
        
        self.M_G_complete = self.d_G_complete.T @ self.Area_inv_extended @ self.d_G_complete
    ...
    def get_F_v(self):
        ...
        for v, indices_extended in tqdm(self.V_map.items(), 
            ...
            self.d1_extended[len(self.F_f) + len(self.F_e) + len(self.F_v), edges] = 1
            ...
            if len(indices_extended) > 2:
                # Add the vertex-face
                self.F_v.append(np.array(E_comb_surround)[:, 0])
        ...
        self.d1_extended = self.d1_extended.tocsr()
            
    def get_homology_basis_extended(self):
        ...
        cycles = []
        self.H_extended = lil_matrix((len(E_co), len(self.E_extended)), dtype=int)
        self.G_H_extended = np.zeros(len(E_co))
        ...
        for i, cotree_edge in tqdm(enumerate(E_co),
            ...
            tree.add_edge(*cotree_edge)
            
            cycle = nx.find_cycle(tree, source=cotree_edge[0])
            
            # Find the cycle created by adding this edge
            cycles.append(cycle)
            
            # Remove the edge again to restore the tree
            tree.remove_edge(*cotree_edge)
            
            e_pre = cycle[-1]
            
            for e in cycle:
                if e in self.E_map:
                    e_extended = self.E_map[e][0]
                elif e[::-1] in self.E_map:
                    e_extended = self.E_map[e[::-1]][0]
                else:
                    raise ValueError("Edge not found in the dictionary")
                
                if np.all(self.V_extended[self.E_extended[e_extended][0]] == self.V[e[0]]):
                    self.H_extended[i, e_extended] = 1
                elif np.all(self.V_extended[self.E_extended[e_extended][0]] == self.V[e[1]]):
                    self.H_extended[i, e_extended] = -1
                else:
                    raise ValueError("Edge not found in the dictionary")

                v_common = np.intersect1d(e, e_pre)[0]
                V_mapped = self.F_v[v_common]
                
                if e_pre in self.E_map:
                    e_pre_extended = self.E_map[e_pre][0]
                elif e_pre[::-1] in self.E_map:
                    e_pre_extended = self.E_map[e_pre[::-1]][0]
                else:
                    raise ValueError("Edge not found in the dictionary")
                
                V_ends = [
                    np.where(np.isin(V_mapped, self.E_extended[e_pre_extended]))[0][0], 
                    np.where(np.isin(V_mapped, self.E_extended[e_extended]))[0][0]
                ]
                if V_ends[0] < V_ends[1]:
                    direction = 1
                else:
                    direction = -1
                    
                v_start = V_ends[::direction][0]
                v_end = V_ends[::direction][1]
                
                for j in range(v_start, v_end):
                    e_fv = self.E_extended_dict[tuple([V_mapped[j], V_mapped[j+1]])]
                    
                    self.H_extended[i, e_fv] = direction
                    self.G_H_extended[i] += self.rotations_across_e_extended[e_fv - len(self.E_twin)] * direction
            
                e_pre = e

            self.G_H_extended[i] = np.mod(self.G_H_extended[i] + np.pi, 2 * np.pi) - np.pi
        
    def get_preconds(self):
        with tqdm(total=1, desc='Computing preconditioning...', leave=False):
            # Theta_extended computation
            self.rotations_across_e_extended = np.array(self.rotations_across_e_extended).squeeze()
            self.G_F_extended = np.concatenate([
                np.zeros(len(self.F_f) + len(self.F_e)),
                self.G_V
            ])
            
            self.A_Theta_extended = vstack([
                self.d1_extended, self.H_extended
            ])
            
            self.A_Theta_extended_KKT_areaweights = bmat([
                [diags(self.area_weights), self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_areaweights = splu(self.A_Theta_extended_KKT_areaweights + 1e-8 * eye(self.A_Theta_extended_KKT_areaweights.shape[0], format='csc'))
            
            self.A_Theta_extended_KKT = bmat([
                [self.M_G_complete, self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT = splu(self.A_Theta_extended_KKT + 1e-8 * eye(self.A_Theta_extended_KKT.shape[0], format='csc'))
            
            self.A_Theta_extended_KKT_ff = bmat([
                [self.M_G_ff, self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_ff = splu(self.A_Theta_extended_KKT_ff + 1e-8 * eye(self.A_Theta_extended_KKT_ff.shape[0], format='csc'))
            
            self.A_Theta_extended_KKT_fe = bmat([
                [self.M_G_fe, self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_fe = splu(self.A_Theta_extended_KKT_fe + 1e-8 * eye(self.A_Theta_extended_KKT_fe.shape[0], format='csc'))
            
            Q = eye(len(self.E_extended), format='lil')
            self.A_Theta_extended_KKT_identity_metric = bmat([
                [Q, self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_identity_metric = splu(self.A_Theta_extended_KKT_identity_metric + 1e-8 * eye(self.A_Theta_extended_KKT_identity_metric.shape[0], format='csc'))
            
            Q = eye(len(self.E_extended), format='lil') * 10
            Q[np.arange(len(self.E_twin)), np.arange(len(self.E_twin))] = 1
            self.A_Theta_extended_KKT_punishing_comb = bmat([
                [Q, self.A_Theta_extended.T],
                [self.A_Theta_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_punishing_comb = splu(self.A_Theta_extended_KKT_punishing_comb + 1e-8 * eye(self.A_Theta_extended_KKT_identity_metric.shape[0], format='csc'))
            
            self.A_Theta_extended_KKT_contracibleonly = bmat([
                [diags(self.area_weights), self.d1_extended.T],
                [self.d1_extended, None]
            ], format='csc')
            self.lu_Theta_extended_KKT_contractibleonly = splu(self.A_Theta_extended_KKT_contracibleonly + 1e-8 * eye(self.A_Theta_extended_KKT_contracibleonly.shape[0], format='csc'))
            
            # Corner phase_extended computation
            G_extended = nx.Graph()
            ...
            for i, e_tree in enumerate(E_tree):
                if tuple(e_tree) in self.E_extended_dict:
                    self.mask_E_tree[i] = self.E_extended_dict[tuple(e_tree)]
                    E_tree[i] = self.E_extended[self.mask_E_tree[i]]
                elif tuple(e_tree[::-1]) in self.E_extended_dict:
                    self.mask_E_tree[i] = self.E_extended_dict[tuple(e_tree[::-1])]
                    E_tree[i] = self.E_extended[self.mask_E_tree[i]]
            
            self.A_Corner_phase_extended = lil_matrix((len(E_tree) + 1, len(self.V_extended)))
            self.A_Corner_phase_extended[np.arange(len(E_tree)), E_tree[:, 0]] = -1
            self.A_Corner_phase_extended[np.arange(len(E_tree)), E_tree[:, 1]] = 1
            self.A_Corner_phase_extended[-1, 0] = 1
            
            self.ATA_Corner_phase_extended = self.A_Corner_phase_extended.T @ self.A_Corner_phase_extended
            
            self.lu_Corner_phase_extended = splu(self.ATA_Corner_phase_extended.tocsc() + 1e-8 * eye(self.ATA_Corner_phase_extended.shape[0], format='csc'))
            
            # Corner scale_extended computation
            self.A_Corner_scale_extended = lil_matrix((len(self.E_extended) + 1, len(self.V_extended)))
            self.A_Corner_scale_extended[np.arange(len(self.E_extended)), self.E_extended[:, 0]] = -1
            self.A_Corner_scale_extended[np.arange(len(self.E_extended)), self.E_extended[:, 1]] = 1
            self.A_Corner_scale_extended[-1, 0] = 1
            
            A_Corner_scale_extended_augmented = bmat([
                [None, self.A_Corner_scale_extended],
                [self.A_Corner_scale_extended.T, None]
            ], format='csc')
            
            self.lu_Corner_scale_extended_augmented = splu(A_Corner_scale_extended_augmented + 1e-8 * eye(A_Corner_scale_extended_augmented.shape[0], format='csc'))
            ...
            self.lu_Coeff = splu(A_Coeff.tocsc())
    ...
    def construct_extended_mesh(self, ratio_twin_to_comb=1):
        ...
        self.d1_extended = lil_matrix((len(self.F) + len(self.E) + len(self.V), 4 * len(self.E)), dtype=float)
        self.rotations_across_e_extended = []
        self.V_map = {v:[] for v in range(self.V.shape[0])}
        self.E_map = {e:[] for e in range(self.E.shape[0])}
        self.E_dict = {}
        self.E_extended_dict = {}
        ...
        for step in tqdm([self.get_V_F_f_E_twin, 
                          self.get_F_e_E_comb, 
                          self.get_F_v, 
                          self.get_homology_basis_extended,
                          self.get_preconds], 
        ...
        self.Corner_scale_extended_harmonic = self.lu_Corner_scale_extended_augmented.solve(
            np.concatenate([
    ...
    def compute_thetas(self):
        ...
        if self.metric_pre != self.metric or np.any(self.I_F != self.I_F_pre) or np.all(self.I_F == 0):
            ...
            if self.metric == 'area weights':
                solution = self.lu_Theta_extended_KKT_areaweights.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'CI':
                solution = self.lu_Theta_extended_KKT.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'ff':
                solution = self.lu_Theta_extended_KKT_ff.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'fe':
                solution = self.lu_Theta_extended_KKT_fe.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'identity':
                solution = self.lu_Theta_extended_KKT_identity_metric.solve(np.concatenate([
                    np.zeros(len(self.E_extended)), 
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'punishing comb':
                solution = self.lu_Theta_extended_KKT_punishing_comb.solve(np.concatenate([
                    np.zeros(len(self.E_extended)), 
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'contractible only':
                solution = self.lu_Theta_extended_KKT_contractibleonly.solve(np.concatenate([
                    np.zeros(len(self.E_extended)), 
                    2 * np.pi * self.I_F - self.G_F_extended
                ]))
            else:
            ...
            self.Theta_extended = solution[:len(self.E_extended)]
            
            self.Theta_extended[len(self.E_twin):] += self.rotations_across_e_extended
            
    def reconstruct_Corner_phase_extended(self, z_init=1):
        if self.metric_pre != self.metric or np.any(self.I_F != self.I_F_pre) or np.all(self.I_F == 0):
            self.Corner_phase_extended = self.lu_Corner_phase_extended.solve(
                self.A_Corner_phase_extended.T @ np.append(
                    self.Theta_extended[self.mask_E_tree], np.angle(z_init)
                )
            )
    ...
    def reconstruct_Corner_scale_extended(self):
        A_Corner_scale_extended = self.A_Corner_scale_extended.copy()
        rank_U = 0
        ...
        for singularity, f in tqdm(zip(self.singularities[self.F_extended_singular < len(self.F_f)], 
            ...
            theta_01 = self.Corner_phase_extended[v1] - self.Corner_phase_extended[v0]
            theta_02 = self.Corner_phase_extended[v2] - self.Corner_phase_extended[v0]
            ...
            x0[v1] = scale_extended_func(a, z1_relative)
            x0[v2] = scale_extended_func(a, z2_relative)
            
            for edge in [edge01, edge12, edge20]:
                v_start, v_end = self.E_twin[edge]
                if x0[v_start] > x0[v_end]:
                    A_Corner_scale_extended[edge, v_end] = -10 * x0[v_start]/x0[v_end]
                    A_Corner_scale_extended[edge, v_start] = 10
                else:
                    A_Corner_scale_extended[edge, v_end] = -10
                    A_Corner_scale_extended[edge, v_start] = 10 * x0[v_end]/x0[v_start]
        ...
        for singularity, f in tqdm(zip(self.singularities[self.F_extended_singular >= len(self.F_f)], 
            ...
            pair_rotation01 = self.rotations_across_e_extended[
            ...
            if np.any(np.isin(self.F_extended_singular[self.F_extended_singular < len(self.F_f)], [face0, face1])):
                ...
                phase_extended_l = [self.Corner_phase_extended[self.E_twin[edge0, 0]], self.Corner_phase_extended[self.E_twin[edge1, 0]] - pair_rotation01 + np.pi]
                phase_extended_r = [self.Corner_phase_extended[self.E_twin[edge0, 1]], self.Corner_phase_extended[self.E_twin[edge1, 1]] - pair_rotation01 + np.pi]
                
                phase_extended_l_used = phase_extended_l[np.argmin([angle_difference(np.exp(1j * arg), np.exp(1j * phase_extended_r[0])) for arg in phase_extended_l])] % (2 * np.pi)
                phase_extended_r_used = phase_extended_r[np.argmin([angle_difference(np.exp(1j * arg), np.exp(1j * phase_extended_l[0])) for arg in phase_extended_r])] % (2 * np.pi)
                
                if abs(phase_extended_l_used - phase_extended_r_used) > np.pi:
                    if phase_extended_l_used > phase_extended_r_used:
                        phase_extended_l_used -= 2 * np.pi
                    else:
                        phase_extended_l_used += 2 * np.pi
                
                phase_extended_singular0 = (phase_extended_r_used + phase_extended_l_used)/2
                phase_extended_singular1 = phase_extended_singular0 + pair_rotation01 + np.pi
                
                if alpha > 1/2:
                    A_Corner_scale_extended[edge0, self.E_twin[edge0, 0]] = 10 * (alpha/(1-alpha)) * \
                        (np.tan(phase_extended_singular0) * np.cos(self.Corner_phase_extended[self.E_twin[edge0, 0]]) - np.sin(self.Corner_phase_extended[self.E_twin[edge0, 0]])) /\
                            (np.sin(self.Corner_phase_extended[self.E_twin[edge0, 1]]) - np.tan(phase_extended_singular0) * np.cos(self.Corner_phase_extended[self.E_twin[edge0, 1]]))
                    A_Corner_scale_extended[edge0, self.E_twin[edge0, 1]] = -10
                    
                    A_Corner_scale_extended[edge1, self.E_twin[edge1, 1]] = -10 * (alpha/(1-alpha)) * \
                        (np.tan(phase_extended_singular1) * np.cos(self.Corner_phase_extended[self.E_twin[edge1, 1]]) - np.sin(self.Corner_phase_extended[self.E_twin[edge1, 1]])) /\
                            (np.sin(self.Corner_phase_extended[self.E_twin[edge1, 0]]) - np.tan(phase_extended_singular1) * np.cos(self.Corner_phase_extended[self.E_twin[edge1, 0]]))
                    A_Corner_scale_extended[edge1, self.E_twin[edge1, 0]] = 10
                            
                else:
                    A_Corner_scale_extended[edge0, self.E_twin[edge0, 1]] = -10 * ((1-alpha)/alpha) * \
                        (np.tan(phase_extended_singular0) * np.cos(self.Corner_phase_extended[self.E_twin[edge0, 1]]) - np.sin(self.Corner_phase_extended[self.E_twin[edge0, 1]])) /\
                            (np.sin(self.Corner_phase_extended[self.E_twin[edge0, 0]]) - np.tan(phase_extended_singular0) * np.cos(self.Corner_phase_extended[self.E_twin[edge0, 0]]))
                    A_Corner_scale_extended[edge0, self.E_twin[edge0, 0]] = 10
                    
                    A_Corner_scale_extended[edge1, self.E_twin[edge1, 0]] = 10 * ((1-alpha)/alpha) * \
                        (np.tan(phase_extended_singular1) * np.cos(self.Corner_phase_extended[self.E_twin[edge1, 0]]) - np.sin(self.Corner_phase_extended[self.E_twin[edge1, 0]])) /\
                            (np.sin(self.Corner_phase_extended[self.E_twin[edge1, 1]]) - np.tan(phase_extended_singular1) * np.cos(self.Corner_phase_extended[self.E_twin[edge1, 1]]))
                    A_Corner_scale_extended[edge1, self.E_twin[edge1, 1]] = -10
                                                           
                self.V_singular_e += [singularity, singularity]
                z0 = np.exp(1j * phase_extended_singular0)
                z1 = np.exp(1j * phase_extended_singular1)
        ...
        print(f'Processing singularities took {time.time() - t0} seconds.')
                
        if len(self.F_f_singular) > 0:
            if np.where(self.mask_removed_v)[0][0] != 0:
                rank_U += 1
                A_Corner_scale_extended[-1, 0] = 0
                A_Corner_scale_extended[-1, np.where(self.mask_removed_v)[0][0]] = 1
                
            # self.Corner_scale_extended = lsqr(A_Corner_scale_extended, np.append(np.zeros(len(self.E_extended)), 1))[0]
            
            delta_A = A_Corner_scale_extended - self.A_Corner_scale_extended
            U, V = compute_uv(delta_A)
            
            # Compute A_inv_b and U_inv
            U_inv = np.column_stack(
                [self.lu_Corner_scale_extended_augmented.solve(
                    np.concatenate([U[:, i], np.zeros(len(self.V_extended))])
                )[len(self.E_extended) + 1:] for i in range(U.shape[1])]
            )
            
            # Compute correction term
            correction = np.linalg.inv(np.eye(rank_U) + V.T @ U_inv) @ (V.T @ self.Corner_scale_extended_harmonic)
            
            # Compute updated solution
            self.Corner_scale_extended = self.Corner_scale_extended_harmonic - U_inv @ correction
        
        else:
            self.Corner_scale_extended = self.Corner_scale_extended_harmonic.copy()
    ...
    def reconstruct_linear_from_Corners(self):
        ...
        '''
        self.U = self.Corner_scale_extended * np.exp(1j * self.Corner_phase_extended)
        ...
        solution = self.lu_Coeff.solve(b_Coeff)
    ...
    def static_field(self, singularities, indices):
        ...
        self.compute_thetas()
        self.reconstruct_Corner_phase_extended()
        # self.reconstruct_Corner_scale_extended()
        self.Corner_scale_extended = 1
    ...
    def dynamic_field(self, F_singular, indices, sample_interval=1, ratio_twin_to_comb=1):
        ...
        def callback():
            ...
            if psim.RadioButton("Identity metric", self.metric == 'identity'):
                ...
                changed = True
            if psim.RadioButton("Punishing combinatorial rotation", self.metric == 'punishing comb'):
                self.metric = 'punishing comb'
                changed = True
            if psim.RadioButton("Only contractible cycles", self.metric == 'contractible only'):
                self.metric = 'contractible only'
                changed = True
