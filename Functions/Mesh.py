...
class Triangle_mesh():
    ...
    def get_V_F_f_E_twin(self):
        ...
        self.Area_inv_extended[:len(self.F), :len(self.F)] = Area_inv
    ...
    def get_preconds(self):
        ...
        M_G_complete = self.d_G_complete.T @ self.Area_inv_extended @ self.d_G_complete
        
        self.A_Theta_extended = vstack([
            self.d1_extended, self.H_extended
        ])
        
        self.A_Theta_extended_KKT_noncontractible = bmat([
            [diags(self.area_weights), self.A_Theta_extended.T],
            [self.A_Theta_extended, None]
        ], format='csc')
        self.lu_Theta_extended_KKT_noncontractible = splu(self.A_Theta_extended_KKT_noncontractible + 1e-8 * eye(self.A_Theta_extended_KKT_noncontractible.shape[0], format='csc'))
        ...
        self.A_Theta_extended_KKT = bmat([
            [M_G_complete, self.d1_extended.T],
            [self.d1_extended, None]
        ], format='csc')
        ...
        self.A_Theta_extended_KKT_ff = bmat([
            [self.M_G_ff, self.d1_extended.T],
            [self.d1_extended, None]
        ], format='csc')
        ...
        self.A_Theta_extended_KKT_fe = bmat([
            [self.M_G_fe, self.d1_extended.T],
            [self.d1_extended, None]
        ], format='csc')
        self.M_Theta_extended_KKT_fe = splu(self.A_Theta_extended_KKT_fe + 1e-8 * eye(self.A_Theta_extended_KKT_fe.shape[0], format='csc'))
        
        Q = eye(len(self.E_extended), format='lil')
        self.A_Theta_extended_KKT_identity_metric = bmat([
            [Q, self.d1_extended.T],
            [self.d1_extended, np.zeros((self.d1_extended.shape[0], self.d1_extended.shape[0]))]
        ], format='csc')
        self.M_Theta_extended_KKT_identity_metric = splu(self.A_Theta_extended_KKT_identity_metric + 1e-8 * eye(self.A_Theta_extended_KKT_identity_metric.shape[0], format='csc'))
        ...
        self.M_Theta_extended_KKT_punishing_comb = splu(self.A_Theta_extended_KKT_punishing_comb + 1e-8 * eye(self.A_Theta_extended_KKT_identity_metric.shape[0], format='csc'))
    ...
    def construct_extended_mesh(self, ratio_twin_to_comb=1):
        ...
        self.Area_inv_extended = lil_matrix((len(self.F) + len(self.E), len(self.F) + len(self.E)), dtype=float)
        ...
        self.Corner_phase_extended_pre = np.zeros(len(self.V_extended))        
        self.Corner_scale_extended_pre = np.zeros(len(self.V_extended))
    ...
    def compute_thetas(self):
        ...
        if self.metric_pre != self.metric or np.any(self.I_F != self.I_F_pre) or np.all(self.I_F == 0):
            ...
            if self.metric == 'noncontractible':
                solution = self.lu_Theta_extended_KKT_noncontractible.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended, 
                    - self.G_H_extended
                ]))
            elif self.metric == 'area weights':
                ...
                solution = self.M_Theta_extended_KKT_fe.solve(np.concatenate([
                    np.zeros(len(self.E_extended)),
                    2 * np.pi * self.I_F - self.G_F_extended
                ]))
            elif self.metric == 'identity':
                solution = self.M_Theta_extended_KKT_identity_metric.solve(np.concatenate([
                    np.zeros(len(self.E_extended)), 
                    2 * np.pi * self.I_F - self.G_F_extended
                ]))
            elif self.metric == 'punishing_comb':
                solution = self.M_Theta_extended_KKT_punishing_comb.solve(np.concatenate([
                    np.zeros(len(self.E_extended)), 
                    2 * np.pi * self.I_F - self.G_F_extended
                ]))
            else:
    ...
    def reconstruct_Corner_phase_extended(self, z_init=1):
        if self.metric_pre != self.metric or np.any(self.I_F != self.I_F_pre) or np.all(self.I_F == 0):
            b = np.append(self.Theta_extended[self.mask_E_tree], np.angle(z_init))
            self.Corner_phase_extended = self.lu_Corner_phase_extended.solve(self.A_Corner_phase_extended.T @ b)
            
            self.Corner_phase_extended_pre = self.Corner_phase_extended.copy()
    ...
    def reconstruct_Corner_scale_extended(self):
        ...
        self.Corner_scale_extended_pre = self.Corner_scale_extended.copy()
    ...
    def dynamic_field(self, F_singular, indices, sample_interval=1, ratio_twin_to_comb=1):
        ...
        def callback():
            ...
            if psim.RadioButton("Noncontractible cycles", self.metric == 'noncontractible'):
                self.metric = 'noncontractible'
                changed = True
